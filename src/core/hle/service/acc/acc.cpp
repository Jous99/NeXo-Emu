...
// a real RS256 JWT. Key comes from NEXO_BAAS_SIGNING_KEY or nexo_baas.pem, else generated.
...
constexpr const char* BaasKeyId = "nexo-baas-key-1";
...
EVP_PKEY* GetBaasSigningKey() {
    static EVP_PKEY* key = []() -> EVP_PKEY* {
        ...
        if (const char* env = std::getenv("NEXO_BAAS_SIGNING_KEY"); env && *env) {
            pem = env;
        } else if (const auto file = Common::FS::ReadStringFromFile(
                       std::filesystem::path{"nexo_baas.pem"}, Common::FS::FileType::TextFile);
                   !file.empty()) {
            pem = file;
        }
        ...
        if (pem.find("BEGIN") != std::string::npos) {
            ...
            if (bio) {
                ...
                BIO_free(bio);
                if (loaded) {
                    LOG_INFO(Service_ACC, "[NeXo] Using the supplied BAAS signing key");
                    return loaded;
                }
                LOG_WARNING(Service_ACC,
                            "[NeXo] BAAS signing key could not be parsed; generating one");
            }
        }
        ...
        const auto auto_key_path =
            Common::FS::GetCitronPath(Common::FS::CitronPath::KeysDir) / "nexo_baas_auto.pem";
        if (const auto existing =
            ...
            if (bio) {
                ...
                BIO_free(bio);
                if (loaded) {
                    LOG_DEBUG(Service_ACC, "[NeXo] Using the persisted auto-generated BAAS signing key");
                    return loaded;
                }
            }
        }
        ...
        if (!generated) {
            LOG_ERROR(Service_ACC, "[NeXo] Failed to generate a BAAS signing key");
            return generated;
        }
            ...
            if (len > 0 && data) {
                ...
                if (written > 0) {
                    LOG_DEBUG(Service_ACC,
                             "[NeXo] Generated and persisted a new BAAS signing key at {}",
...
std::string SignRs256(std::string_view signing_input) {
    ...
    if (signature.empty()) {
        LOG_ERROR(Service_ACC, "[NeXo] Failed to sign the BAAS id_token");
    }
...
std::string BuildIdToken() {
    ...
    // [NeXo] Ride the signed nx2 token in the "nnex" claim so the auth server can
    ...
    if (Common::NeXoAccount::IsLinked()) {
        const std::string tok = Common::NeXoAccount::GetToken();
        if (!tok.empty()) {
            nnex_claim = fmt::format(R"("nnex":"{}",)", tok);
        }
    }

    // [NeXo] The console's own nnAccount module expects a "nintendo" claim dictionary
    ...
    // side has nothing to complain about. Matches Ryujinx-NeXo's own fix for the exact
    ...
    // Ryujinx-NeXo's own token (a single deviceId feeding both places).
...
std::vector<u8> GetIdTokenBytes() {
    ...
    // [NeXo] Rebuild whenever the linked account changes (sign-in/sign-out), not just on
    ...
    const u64 generation = Common::NeXoAccount::GetGeneration();
    ...
    if (cached.empty() || now >= expiry || generation != cached_generation) {
        ...
        LOG_INFO(Service_ACC, "[NeXo] Issued a signed BAAS id_token ({} bytes)", cached.size());
    }
...
static void SeedImageFromNeXoIfMissing(const Common::UUID& uuid) {
    ...
    const auto jpeg = Common::NeXoAvatar::GetSelfJPEG();
    if (jpeg.empty()) {
        return;
    }
...
private:
    ...
    void GetNintendoAccountUserResourceCache(HLERequestContext& ctx) {
        LOG_DEBUG(Service_ACC, "[NeXo] GetNintendoAccountUserResourceCache called");
...
protected:
    ...
    void LoadImage(HLERequestContext& ctx) {
        ...
        SeedImageFromNeXoIfMissing(user_id);
...
public:
    ...
    void LoadIdTokenCache(HLERequestContext& ctx) {
        ...
        LOG_INFO(Service_ACC, "[NeXo] Providing BAAS ID token in async interface ({} bytes)",
                 token_bytes.size());
...
private:
    ...
    void GetNetworkServiceLicenseKind(HLERequestContext& ctx) {
        LOG_INFO(Service_ACC, "[NeXo] GetNetworkServiceLicenseKind called -> returning Subscribed (2)");
        IPC::ResponseBuilder rb{ctx, 3};
        rb.Push(ResultSuccess);
        rb.Push<u32>(2);
    }
...
private:
    u64 GetEffectivePid() const {
        const u64 raw_pid = []() -> u64 {
            if (const u64 linked = Common::NeXoAccount::GetPid(); linked != 0) {
                return linked;
            }
            std::string pid_setting = Settings::values.nexo_pid.GetValue();
            if (!pid_setting.empty()) {
                try {
                    return std::stoull(pid_setting);
                } catch (...) {}
            }
            const char* pid_env = std::getenv("NEXO_PID");
            if (pid_env && *pid_env) {
                try {
        ...
        // [NeXo] Version gate: refuse to present a real PID for a title whose installed
        // version doesn't match what NeXo's servers require, reusing the exact same
        ...
        if (NeXo::CompatibleTitles::Table().contains(program_id)) {
            const FileSys::PatchManager pm{program_id, system.GetFileSystemController(),
                                           system.GetContentProvider()};
            const auto metadata = pm.GetControlMetadata();
            const std::string installed_version =
                metadata.first != nullptr ? metadata.first->GetVersionString() : std::string{};
            if (!NeXo::CompatibleTitles::IsVersionOk(program_id, installed_version)) {
                LOG_WARNING(Service_ACC,
                            "[NeXo] Refusing online PID: installed version doesn't match "
    ...
    void GetAccountId(HLERequestContext& ctx) {
        ...
        LOG_DEBUG(Service_ACC, "[NeXo] Returning the linked account's Network ID");
    ...
    void LoadIdTokenCache(HLERequestContext& ctx) {
        const std::vector<u8> token_bytes = GetIdTokenBytes();
        LOG_INFO(Service_ACC, "[NeXo] Providing BAAS ID token ({} bytes)", token_bytes.size());
    ...
    void GetNintendoAccountUserResourceCacheForApplication(HLERequestContext& ctx) {
        u64 account_id = GetEffectivePid();
        LOG_DEBUG(Service_ACC, "[NeXo] GetNintendoAccountUserResourceCacheForApplication called");
    ...
    void LoadNetworkServiceLicenseKindAsync(HLERequestContext& ctx) {
        LOG_INFO(Service_ACC, "[NeXo] LoadNetworkServiceLicenseKindAsync called");
