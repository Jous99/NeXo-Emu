...
ProfileManager::ProfileManager() {
    ...
    if (user_count == 0) {
        const std::string default_username = Common::NeXoAccount::IsLinked()
                                                  ? Common::NeXoAccount::GetUsername()
                                                  : "citron";
    ...
    // [NeXo] The default/existing profile username has no relation to the linked NeXo
    ...
    if (Common::NeXoAccount::IsLinked()) {
        ...
        if (uuid && GetProfileBase(*uuid, base)) {
            const auto username = Common::NeXoAccount::GetUsername();
