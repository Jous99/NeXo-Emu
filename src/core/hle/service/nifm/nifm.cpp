...
public:
    explicit IRequest(Core::System& system_)
        ...
        // [NeXo] ResolveState() (and the event1->Signal() it can trigger via UpdateState)
...
private:
    ...
    // [NeXo] Callable from either the IPC-dispatch thread (guest calling GetRequestState/
    ...
    void UpdateState(RequestState new_state) {
        state = new_state;
        event1->Signal();
        // [NeXo] event2 (the second handle GetSystemEventReadableHandles hands the guest) was
