...
public:
            ...
            if (err == Network::Errno::SUCCESS) {
                ...
                if (effective_host.empty() || effective_host == ip_str) {
                    ...
                    if (!last_host.empty()) {
                        ...
                        LOG_INFO(Service_SSL, "[NeXo] Recovered host '{}' for IP {}",
    ...
    Result SetVerifyOption(u32 verify_option) override {
        // [NeXo] Always bypass, matching the OpenSSL backend; a title requesting real
        // verification would otherwise fail the self-signed cert against the Windows trust store.
        skip_cert_verification = true;
        LOG_DEBUG(Service_SSL, "SetVerifyOption: option={}, bypassing cert verification for NeXo",
                  verify_option);
        return ResultSuccess;
    }
    ...
    Result CallInitializeSecurityContext() {
        ...
        // [NeXo] NEX (MK8/Splatoon 2) uses PRUDP over WebSocket which requires HTTP/1.1
