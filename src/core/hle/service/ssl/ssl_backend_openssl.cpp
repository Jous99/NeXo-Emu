...
// [NeXo] NPLN (Splatoon 3's gRPC/HTTP2 stack) needs h2; everyone else needs http/1.1-only
...
public:
            ...
            if (err == Network::Errno::SUCCESS) {
                ...
                if (effective_host.empty() || effective_host == ip_str) {
                    ...
                    if (!last_host.empty()) {
                        ...
                        LOG_INFO(Service_SSL, "[NeXo] Recovered host '{}' for IP {}", effective_host, ip_str);
                    }
                }
            }
        }
    ...
    Result SetVerifyOption(u32 verify_option) override {
        // [NeXo] Always bypass certificate verification for self-hosted/redirected servers
        skip_cert_verification = true;
        LOG_DEBUG(Service_SSL, "SetVerifyOption: option={}, bypassing cert verification for NeXo", verify_option);
    ...
    Result DoHandshake(std::span<const std::string> requested_alpn_protos) override {
        ...
        if (socket && !SSL_get_servername(ssl, TLSEXT_NAMETYPE_host_name)) {
            ...
            if (err == Network::Errno::SUCCESS) {
                ...
                if (!last_host.empty()) {
                    ...
                    LOG_INFO(Service_SSL, "[NeXo] DoHandshake SNI injection host '{}' for IP {}", last_host, ip_str);
        ...
        if (!npln_wire.empty()) {
            LOG_INFO(Service_SSL, "[NeXo] NPLN host '{}': honoring game-requested ALPN ({} byte(s))",
        ...
        // Bypassing verification for NeXo compatibility
        SSL_set_verify(ssl, SSL_VERIFY_NONE, nullptr);
