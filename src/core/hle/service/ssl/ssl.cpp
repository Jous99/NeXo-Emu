...
private:
    ...
    void Peek(HLERequestContext& ctx) {
        // [NeXo] Nintendo's libcurl checks connection liveness via Peek.
    ...
    void SetNextAlpnProto(HLERequestContext& ctx) {
        ...
        // [NeXo] Wire format: repeated [1-byte len][name]. Passed to DoHandshakeImpl.
...
public:
    explicit ISslServiceForSystem(Core::System& system_)
        ...
        // clang-format off
        // [NeXo] Shares command IDs with ISslService (0-9); system-only extensions start
...
private:
    ...
    Result GetCertificateBufSize(
        ...
        LOG_INFO(Service_SSL, "[NeXo][DIAG] GetCertificateBufSize -> size={} num_entries={} requested_ids={}",
    ...
    Result GetCertificates(Out<u32> out_num_entries, OutBuffer<BufferAttr_HipcMapAlias> out_buffer,
        ...
        LOG_INFO(Service_SSL, "[NeXo][DIAG] GetCertificates -> num_entries={} requested_ids={}",
