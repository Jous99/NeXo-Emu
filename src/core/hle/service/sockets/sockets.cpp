...
void LoopProcess(Core::System& system) {
    ...
    // [NeXo] No extra worker threads: matches Ryujinx's single-threaded Bsd service, which
    ...
    // [NeXo] See sockets.h's declaration comment on SetBsdDeferralEvent.
    Kernel::KEvent* deferral_event{};
    server_manager->ManageDeferral(&deferral_event);
    SetBsdDeferralEvent(deferral_event);

    // [NeXo] Nothing else re-signals this event when a *regular* socket (as opposed to an
    ...
    //
    // [NeXo] 50ms -> 1ms, matching Ryujinx-NeXo's own measured fix for this identical
