...
// [NeXo] See sfdnsres.h's declaration comment. Real bug this exists for (Splatoon 3,
// confirmed via Ryujinx-NeXo hitting the identical failure on the same guest binary):
// its gRPC channel resolves a redirected NeXo hostname correctly here, then later loses
...
// [NeXo] Resolves `host` on a detached worker and waits at most `timeout_ms` for it. The
...
static std::optional<std::string> GetNeXoRedirectIp(const std::string& host) {
    if (!Settings::values.enable_nexo.GetValue()) {
        return std::nullopt;
    }

    const std::string server_ip =
        GetConfiguredIp(Settings::values.nexo_server_ip.GetValue(), "NEXO_SERVER_IP");
    const std::string nat_ip =
        GetConfiguredIp(Settings::values.nexo_nat_ip.GetValue(), "NEXO_NAT_IP");

    if (host.starts_with("nncs2-") && host.ends_with(".n.n.srv.nintendo.net")) {
        LOG_INFO(Service, "[NeXo] Redirecting NAT check host '{}' -> '{}'", host, nat_ip);
        return nat_ip;
    }

    // [NeXo] Mario Tennis Aces (NEX server id g23932a00) resolves through a real GeoDNS
    ...
    if (host == "nintendo.net" || host.ends_with(".nintendo.net") ||
        ...
        LOG_INFO(Service, "[NeXo] Redirecting Nintendo host '{}' -> '{}'", host, server_ip);
        return server_ip;
    }
...
// [NeXo] Debug-only tap: redirects an "npln" host straight to a local TLS-terminating
// proxy instead of production, for protocol inspection. Independent of enable_nexo (this
// isn't a NeXo-server redirect, just a temporary debugging aid) -- unset by default, so it
// never affects a normal run. NEXO_S3_DEBUG_PROXY_IP=<ip> to enable.
static std::optional<std::string> GetNplnDebugProxyIp(const std::string& host) {
    if (Common::ToLower(host).find("npln") == std::string::npos) {
        return std::nullopt;
    }
    const char* env = std::getenv("NEXO_S3_DEBUG_PROXY_IP");
    if (!env || !*env) {
        return std::nullopt;
    }
    LOG_INFO(Service, "[NeXo] Redirecting npln host '{}' -> debug proxy '{}'", host, env);
    return std::string(env);
}

// [NeXo] Optional delay before the first "npln" host resolution, against a hypothesized
...
// NEXO_NPLN_DELAY_MS opts back into a fixed wait if ever needed.
...
static void MaybeDelayNplnInit(const std::string& host) {
    ...
    std::call_once(g_npln_delay_once, [] {
        int max_wait_ms = 0;
        if (const char* env = std::getenv("NEXO_NPLN_DELAY_MS"); env && *env) {
            try {
        ...
        LOG_INFO(Service,
                 "[NeXo] Holding the first npln host resolution until the JIT/shader-compile "
        ...
        LOG_INFO(Service, "[NeXo] npln hold finished after {} ms", max_wait_ms);

        // [NeXo][DIAG] Arm a short window during which any finite, non-trivial
        // WaitSynchronization timeout gets logged -- see nexo_deadline_watch.h. This is
        ...
        Kernel::Svc::ArmNeXoDeadlineWatch(90000);
        LOG_INFO(Service, "[NeXo][DIAG] Deadline watch armed for 90000 ms");
    });
}
...
static std::pair<u32, GetAddrInfoError> GetHostByNameRequestImpl(HLERequestContext& ctx) {
    ...
    LOG_INFO(Service, "[NeXo] DNS resolve (GetHostByName) requested: host={}", host);

    // [NeXo] See MaybeDelayNplnInit's declaration comment.
    ...
    auto redirect = GetNeXoRedirectIp(host);
...
static std::vector<u8> SerializeAddrInfo(const std::vector<Network::AddrInfo>& vec,
    ...
    for (const Network::AddrInfo& addrinfo : vec) {
        ...
        // Ryujinx-NeXo's AddrInfo4.Length fix (was sizeof(Array4<byte>)=4, needed to be
...
static std::pair<u32, GetAddrInfoError> GetAddrInfoRequestImpl(HLERequestContext& ctx) {
    ...
    LOG_INFO(Service, "[NeXo] DNS resolve (GetAddrInfo) requested: host={}", host);

    // [NeXo] See MaybeDelayNplnInit's declaration comment.
    MaybeDelayNplnInit(host);

    // [NeXo] A literal IP has nothing to resolve -- return it as-is, before any of the
    ...
    if (Network::IPv4Address literal_ip; Network::TryParseIPv4Literal(host, literal_ip)) {
        LOG_DEBUG(Service, "[NeXo] Host '{}' is already a literal address: returned as-is",
                  host);
        ...
        // Deliberately no SetLastHostForIp here, matching Ryujinx-NeXo's own fix -- a
    ...
    if (!redirect.has_value()) {
        redirect = GetNeXoRedirectIp(host);
    }
    if (redirect.has_value()) {
        query_host = *redirect;
    } else if (blocked_domains.find(host) != blocked_domains.end()) {
    ...
    if (redirect.has_value()) {
        // [NeXo] GetAddressInfo never requests AI_CANONNAME, so canon_name comes back
        ...
        // [NeXo] Port-keyed recovery for gRPC-based titles (Splatoon 3) that lose this
