...
static std::string ResolveImpl(Core::System& system, const std::string& fqdn_in) {
    // Use Raptor Network URL resolver: rewrites Nintendo server FQDNs to Raptor
    // endpoints, and blocks any Nintendo servers without a rewrite rule.
    const std::string resolved = system.GetOnlineInitiator().ResolveUrl(fqdn_in, true);
    if (resolved != fqdn_in) {
        LOG_INFO(Service, "NSD Raptor resolve: '{}' -> '{}'", fqdn_in, resolved);
    }
    return resolved;
}

static Result ResolveCommon(Core::System& system, const std::string& fqdn_in,
                             std::array<char, 0x100>& fqdn_out) {
    const auto res = ResolveImpl(system, fqdn_in);
...
void NSD::Resolve(HLERequestContext& ctx) {
    ...
    std::array<char, 0x100> fqdn_out{};
    const Result res = ResolveCommon(system, fqdn_in, fqdn_out);
