...
Result IApplicationFunctions::EnsureSaveData(Out<u64> out_size, Common::UUID user_id) {
    ...
    // [NeXo] A real console also provisions the title's BCAT delivery-cache storage here,
    // proactively, the first time EnsureSaveData runs -- not just the account save above. Ryujinx
    // never did this at all (upstream bug, not NeXo-specific) until it hit Splatoon 3, which
    ...
    // this went unnoticed for years. Matches Ryujinx-NeXo's own fix (df268d0/e7d39fa,
