...
Result WaitProcessWideKeyAtomic(Core::System& system, u64 address, u64 cv_key, u32 tag,
    ...
    // [NeXo][DIAG] The last uninstrumented Horizon OS blocking primitive this session --
    ...
    const bool diag_cv = IsNeXoDeadlineWatchActive();
    std::chrono::steady_clock::time_point diag_cv_start{};
    if (diag_cv) {
        diag_cv_start = std::chrono::steady_clock::now();
        LOG_INFO(Kernel_SVC, "[NeXo][DIAG] WaitProcessWideKeyAtomic starting, cv_key={:X}, "
                             "timeout_ns={}",
                 cv_key, timeout_ns);
    }
    ...
    if (diag_cv) {
        const auto elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                                     ...
                                     .count();
        LOG_INFO(Kernel_SVC,
                 "[NeXo][DIAG] WaitProcessWideKeyAtomic resolved after {}ms wall-clock, "
...
void SignalProcessWideKey(Core::System& system, u64 cv_key, s32 count) {
    ...
    if (IsNeXoDeadlineWatchActive()) {
        LOG_INFO(Kernel_SVC, "[NeXo][DIAG] SignalProcessWideKey cv_key={:X}, count={}", cv_key,
                 count);
    }
