...
Result CreateThread(Core::System& system, Handle* out_handle, u64 entry_point, u64 arg,
    ...
    // [NeXo][DIAG] Every guest thread is spawned through the same SDK trampoline, so
    ...
    if (const auto* caller = GetCurrentThreadPointer(system.Kernel())) {
        ...
        LOG_INFO(Kernel_SVC,
                 "[NeXo][DIAG] CreateThread entry_point=0x{:08X} caller backtrace:{}",
...
void SleepThread(Core::System& system, s64 ns) {
    ...
    // [NeXo][DIAG] SleepThread isn't covered by the WaitSynchronization deadline watch --
    ...
    const bool diag_sleep = ns > 50'000'000 && IsNeXoDeadlineWatchActive();
    std::chrono::steady_clock::time_point diag_sleep_start{};
    if (diag_sleep) {
        diag_sleep_start = std::chrono::steady_clock::now();
        LOG_INFO(Kernel_SVC, "[NeXo][DIAG] SleepThread starting, requested={}ms",
                 ns / 1'000'000);
    }
    ...
    if (ns > 0) {
        ...
        if (diag_sleep) {
            const auto elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                                         ...
                                         .count();
            LOG_INFO(Kernel_SVC, "[NeXo][DIAG] SleepThread resolved after {}ms wall-clock",
                     elapsed_ms);
        }
