...
#include "core/loader/nexo_s3_patches.h"
...
std::optional<VAddr> AppLoader_NSO::LoadModule(Kernel::KProcess& process, Core::System& system,
    ...
    // [NeXo] Splatoon 3's built-in patches (certificate-pinning bypass, peer hostname fix)
    ...
    LOG_INFO(Loader, "[NeXo][DIAG] S3 patch check: module={}, pm_valid={}, title_id={:016X}",
    ...
    if (pm && pm->GetTitleID() == 0x0100C2500FC20000ULL) {
        ...
        if (pi_header.size() > 0x0014DD84) {
            LOG_INFO(Loader, "[NeXo][DIAG] bytes at 0x157B20/0x14E1B0/0x14DD80 before patch: "
        ...
        pi_header =
            Loader::NeXoS3Patches::ApplyIfMatch(nso_header.build_id, std::move(pi_header));

        if (pi_header.size() > 0x0014DD84) {
            LOG_INFO(Loader, "[NeXo][DIAG] bytes at 0x157B20/0x14E1B0/0x14DD80 after patch: "
                              "{:02X} {:02X} {:02X} {:02X} | {:02X} {:02X} {:02X} {:02X} | "
                              "{:02X} {:02X} {:02X} {:02X}",
                      pi_header[0x00157B20], pi_header[0x00157B21], pi_header[0x00157B22],
                      pi_header[0x00157B23], pi_header[0x0014E1B0], pi_header[0x0014E1B1],
        ...
        if (pi_header.size() >= sizeof(NSOHeader) &&
            ...
            LOG_ERROR(Loader, "[NeXo] Splatoon 3 built-in patch changed the image size "
