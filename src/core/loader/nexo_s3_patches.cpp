...
// Bytes match NeXoNetwork/Ryujinx-NeXo's NeXoS3Patches.cs. Offsets are Ryujinx's
...
std::vector<u8> ApplyOne(std::vector<u8> nso, std::span<const u8> ips_bytes,
    ...
    if (patched == nullptr) {
        LOG_ERROR(Loader, "[NeXo] Splatoon 3: {} patch failed to apply", what);
        return nso;
    }
...
std::vector<u8> ApplyIfMatch(const std::array<u8, 0x20>& build_id, std::vector<u8> nso) {
    ...
    for (const auto& known : kKnownBuilds) {
        ...
        LOG_INFO(Loader, "[NeXo] Splatoon 3: {} built-in patch(es) applied (build {})",
