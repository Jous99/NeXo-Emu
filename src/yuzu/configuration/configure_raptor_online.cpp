...
#include "ui_configure_raptor_online.h"
#include "yuzu/configuration/configure_raptor_online.h"
...
void ConfigureOnline::SetConfiguration() {
    ...
    ui->groupBoxOnline->setHidden(Settings::values.raptor_token.GetValue().empty());
    ui->groupBoxOffline->setHidden(!Settings::values.raptor_token.GetValue().empty());
}
...
void ConfigureOnline::AccountLogout() {
    Settings::values.raptor_token.SetValue("");
    online_initiator->Connect();
    OnLoginVerified();
}
