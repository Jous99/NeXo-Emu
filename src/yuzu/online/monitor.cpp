...
OnlineStatusMonitor::OnlineStatusMonitor(Core::OnlineInitiator& online_initiator_)
    ...
    if (Settings::values.raptor_token.GetValue().empty()) {
        setChecked(false);
        ...
        if (redirect_token.empty()) {
            ...
            if (response) {
                if (response->status == 200) {
                    Settings::values.raptor_token.SetValue(response->body);
                    emit SaveConfig();
...
void MonitorWorker::UpdateInitiator() {
    ...
    if (!online_initiator.IsConnected()) {
        status = Settings::values.raptor_token.GetValue().empty() ? &NOT_LOGGED_IN : &INVALID_TOKEN;
        return;
    }
...
void MonitorWorker::UpdateUserStatus() {
    ...
    headers.emplace("Authorization", fmt::format("Bearer {}", Settings::values.raptor_token.GetValue()));
    headers.emplace("R-ClientId", "nexoemu");
