...
struct Client::Impl {
    ...
    WebResult GenericRequest(const std::string& method, const std::string& path,
        ...
        // NeXoEmu: add Raptor Network hardware identification headers
        params.emplace(std::string("R-HardwareId"), Common::GetRaptorHardwareID());
        params.emplace(std::string("R-ClientId"), std::string("nexoemu"));
        if (method != "GET") {
