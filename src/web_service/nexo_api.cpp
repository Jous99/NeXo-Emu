...
namespace WebService::NeXoApi {

namespace {

constexpr const char* CanonicalUrl = "https://nexonetwork.space";
constexpr const char* ClientId = "nexo-emulator";
    ...
    const std::string inner =
        ok ? "<h1 style='color:#33e86b'>Signed in</h1><p>You can close this tab and go back to "
           ...
           : "<h1 style='color:#ff8a8a'>Sign-in cancelled</h1><p>Go back to citron and try "
             "again.</p>";
    return "<!doctype html><meta charset=utf-8><title>NeXo</title>"
...
std::optional<std::string> SanitizeBaseUrl(std::string raw) {
    ...
    if (host != "nexonetwork.space" && !host.ends_with(".nexonetwork.space")) {
        return std::nullopt;
    }
    return raw;
}
...
bool ClearSessionIfRejected(const httplib::Result& result) {
    ...
    LOG_WARNING(WebService, "NeXo rejected the stored account token; signing out");
...
std::string BaseUrl() {
    static const std::string url = [] {
        const char* env = std::getenv("NEXO_API");
        if (env && *env) {
            if (const auto sanitized = SanitizeBaseUrl(env)) {
                return *sanitized;
            }
            LOG_WARNING(WebService,
                        "Ignoring NEXO_API=\"{}\": only https on nexo.network, or "
...
LoginResult SignInWithBrowser(const std::function<void(const std::string&)>& open_url) {
    ...
    if (!result) {
        out.error = "Could not reach the NeXo account server.";
        const long verify_result = client.get_openssl_verify_result();
...
OnlineStatus GetOnlineStatus() {
    ...
    const std::string token = Common::NeXoAccount::GetToken();
    if (token.empty()) {
        return out;
    }
...
std::vector<u8> DownloadBcatSeed(const std::string& title_id_hex) {
    ...
    if (!result || result->status != 200) {
        LOG_WARNING(WebService, "NeXo BCAT seed download failed (HTTP {})",
...
std::optional<std::vector<u8>> PullSave(const std::string& title_id_hex) {
    ...
    if (result->status != 200) {
        LOG_WARNING(WebService, "NeXo save pull failed (HTTP {})", result->status);
        return std::nullopt;
    }
...
std::string PushSave(const std::string& title_id_hex, std::span<const u8> data) {
    ...
    if (!result) {
        return "Could not reach the NeXo account server.";
    }
    if (result->status != 200) {
...
std::map<std::string, int> GetOnlineCounts() {
    ...
    if (!result || result->status != 200) {
        LOG_WARNING(WebService, "NeXo online-counts fetch failed (HTTP {})",
                    result ? result->status : 0);
        return out;
    }
    try {
...
void SyncHistory(const std::vector<HistoryEntry>& entries) {
    ...
    if (!result || result->status != 200) {
        LOG_WARNING(WebService, "NeXo history sync failed (HTTP {})",
                    result ? result->status : 0);
        return;
    }
    LOG_INFO(WebService, "NeXo history synced ({} title(s))", entries.size());
}
...
Friend ParseFriend(const nlohmann::json& json) {
    ...
    // [NeXo] TEMP: unconditional (not gated on "presence" being present) so we can see
    ...
    LOG_DEBUG(WebService, "[NeXo] ParseFriend pid={} name='{}' status={} app_field={}",
...
void PushProfileName(const std::string& name) {
    ...
    if (!result || result->status != 200) {
        LOG_WARNING(WebService, "NeXo profile name push failed (HTTP {})",
                    result ? result->status : 0);
    }
}
...
void PushPresence(s32 status, const std::string& app_field, const std::string& app_id,
    ...
    if (!result || result->status != 200) {
        LOG_WARNING(WebService, "NeXo presence push failed (HTTP {})",
                    result ? result->status : 0);
        return;
    }
    LOG_DEBUG(WebService, "NeXo presence published (status={})", status);
}
