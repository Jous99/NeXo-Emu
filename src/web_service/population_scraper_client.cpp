...
std::optional<std::string> FetchNeXoPopulationJson() {
    ...
    const auto result = client.Get("/CollectingW/nexo-population/main/data/population.json");
    if (!result || result->status != 200) {
        LOG_WARNING(WebService, "nexo-population fetch failed (HTTP {})",
