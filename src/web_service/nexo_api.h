...
// The API base url. NEXO_API overrides it; only https or loopback is accepted, because this
...
struct LobbyPlayer {
    u64 pid = 0;
    std::string name;
    bool known = false; // false when pid isn't a known NeXo account (can't add/report)
...
// signal (Ryujinx-NeXo hits the same server and has the same finding), so the caller should
