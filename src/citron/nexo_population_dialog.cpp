...
NeXoPopulationDialog::NeXoPopulationDialog(QWidget* parent) : QDialog(parent) {
    setWindowTitle(tr("NeXo Population"));
    setFixedSize(561, 367);

    auto* layout = new QVBoxLayout(this);

    auto* description = new QLabel(
        tr("Average NeXo online population by hour, sourced from a community-run scraper of "
...
void NeXoPopulationDialog::RefreshChart(int tab) {
    const auto title_ids = TitlesForTab(tab);

    NeXo::PopulationHistory::DayProfile summed{};
    int live_total = 0;
    bool any_data = false;

    for (u64 id : title_ids) {
        live_total += NeXo::OnlineCounts::For(id);
        const auto profile = NeXo::PopulationHistory::For(id);
        if (!profile) {
            continue;
        }
        any_data = true;
    ...
    const std::string updated = NeXo::PopulationHistory::LastUpdatedUtc();
