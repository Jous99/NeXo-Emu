...
// see UISettings::values.nexo_notification_corner). No invite action, dismisses on timeout
// or click. Only shown while main_window is active and not minimized -- see NeXoToast::Show.
class NeXoToast : public QWidget {
    Q_OBJECT
...
public:
    ...
    explicit NeXoToast(QWidget* main_window);
    ~NeXoToast() override;
...
signals:
    void clicked(NeXoToast::Kind kind);
