...
namespace NeXo::PopulationHistory {
...
std::filesystem::path CachePath() {
    return Common::FS::GetCitronPath(Common::FS::CitronPath::CacheDir) / "nexo_population.json";
}
...
void Refresh() {
    ...
    std::thread{[] {
        const auto body = WebService::FetchNeXoPopulationJson();
        if (body) {
            try {
                ...
                LOG_WARNING(Frontend, "nexo-population: unexpected response: {}", e.what());
            }
        }
