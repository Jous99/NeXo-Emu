...
namespace {
// Sizes at scale=1.0 (see NeXoToast::ComputeScale) -- a 1920-wide screen or narrower. This is
...
NeXoToast::NeXoToast(QWidget* main_window_)
    ...
    connect(&auto_hide_timer, &QTimer::timeout, this, &NeXoToast::HideAnimated);
...
NeXoToast::~NeXoToast() = default;

bool NeXoToast::eventFilter(QObject* watched, QEvent* event) {
...
float NeXoToast::ComputeScale() const {
...
NeXoToast::Corner NeXoToast::CurrentCorner() const {
    return static_cast<Corner>(std::clamp(
        UISettings::values.nexo_notification_corner.GetValue(), 0, 3));
}

void NeXoToast::Show(const QString& headline, const QString& detail,
                         const QString& avatar_base64, Kind kind_) {
    if (!UISettings::values.nexo_notifications_enabled.GetValue()) {
        return;
    }
    if (!main_window) {
        return;
    ...
    line1 = headline;
    line2 = detail;
    avatar = NeXo::AvatarCache::Get(headline.toStdString(), avatar_base64.toStdString(),
...
void NeXoToast::mousePressEvent(QMouseEvent* event) {
...
void NeXoToast::paintEvent(QPaintEvent*) {
...
QColor NeXoToast::AccentColor() const {
    switch (kind) {
    case Kind::Online:
...
QString NeXoToast::CategoryLabel() const {
    switch (kind) {
    case Kind::Online:
