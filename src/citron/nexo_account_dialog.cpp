...
#include "common/nexo_outgoing_requests.h"
#include "common/settings.h"
#include "citron/nexo_account_dialog.h"
...
class NeXoToggleSwitch : public QWidget {
public:
    explicit NeXoToggleSwitch(QWidget* parent = nullptr) : QWidget(parent) {
        setFixedSize(46, 26);
        setCursor(Qt::PointingHandCursor);
        setFocusPolicy(Qt::StrongFocus);
    }
...
class NeXoDropdownRow : public ClickableFrame {
public:
    NeXoDropdownRow(const QString& text, bool selected, QWidget* parent = nullptr)
...
class NeXoDropdown : public QWidget {
public:
    explicit NeXoDropdown(QWidget* parent = nullptr) : QWidget(parent) {
        setFixedHeight(34);
        setMinimumWidth(160);
        setCursor(Qt::PointingHandCursor);
        setFocusPolicy(Qt::StrongFocus);
    ...
    void OpenPopup() {
        ...
        layout->setSpacing(2);
        NeXoDropdownRow* first_row = nullptr;
        for (int i = 0; i < values.size(); ++i) {
            auto* row = new NeXoDropdownRow(values[i], i == index, popup);
...
class NeXoSignalIcon : public QWidget {
public:
    explicit NeXoSignalIcon(QWidget* parent = nullptr) : QWidget(parent) {
        setFixedSize(26, 16);
        auto* timer = new QTimer(this);
...
class NeXoPageNavArrow : public QWidget {
public:
    enum class Side { Left, Right };

    explicit NeXoPageNavArrow(Side side_, QWidget* parent = nullptr)
...
NeXoAccountDialog::NeXoAccountDialog(NeXoController* controller_,
    ...
    setWindowTitle(tr("NeXo Account"));
    ...
    connect(edit_name_button, &QToolButton::clicked, this,
           &NeXoAccountDialog::OnEditUsername);
    ...
    background_menu->addAction(tr("Choose Background Image..."), this,
                               &NeXoAccountDialog::OnChangeBackground);
    background_menu->addAction(tr("Remove Background"), this,
                               &NeXoAccountDialog::OnRemoveBackground);
    ...
    static_cast<ClickableFrame*>(dash_requests_card)->on_click = [this] {
        // [NeXo] Requests live on the Friends page now.
    ...
    auto* notifications_toggle = new NeXoToggleSwitch;
    notifications_toggle->SetChecked(UISettings::values.nexo_notifications_enabled.GetValue());
    notifications_toggle->on_toggled = [](bool checked) {
        UISettings::values.nexo_notifications_enabled.SetValue(checked);
    };
    ...
    auto* notification_corner = new NeXoDropdown;
    notification_corner->SetValues({tr("Top Right"), tr("Top Left"), tr("Bottom Right"),
                                    tr("Bottom Left")},
                                   std::clamp(UISettings::values.nexo_notification_corner.GetValue(), 0, 3));
    ...
    notification_corner->on_changed = [](int index) {
        UISettings::values.nexo_notification_corner.SetValue(index);
    };
    ...
    friend_delegate = new NeXoFriendDelegate(friends_view, this);
    friends_view->setItemDelegate(friend_delegate);
    connect(friends_view, &QListView::clicked, this, &NeXoAccountDialog::OnFriendsViewClicked);
    connect(friend_search, &QLineEdit::textChanged, this, &NeXoAccountDialog::ApplyFriendFilter);
    ...
    // [NeXo] Friends and Requests, side-by-side on one page.
    ...
    request_delegate = new NeXoFriendDelegate(requests_view, this);
    requests_view->setItemDelegate(request_delegate);
    connect(requests_view, &QListView::clicked, this, &NeXoAccountDialog::OnFriendsViewClicked);
    ...
    history_view->setItemDelegate(new NeXoHistoryDelegate(history_view, this));
    connect(history_view, &QListView::clicked, this, &NeXoAccountDialog::OnHistoryViewClicked);
    ...
    lobby_delegate = new NeXoFriendDelegate(lobby_view, this);
    ...
    connect(lobby_view, &QListView::clicked, this, &NeXoAccountDialog::OnPlayersViewClicked);
    ...
    recent_players_delegate = new NeXoFriendDelegate(recent_players_view, this);
    ...
    connect(recent_players_view, &QListView::clicked, this,
           &NeXoAccountDialog::OnPlayersViewClicked);
    ...
    // [NeXo] Current Lobby and Recently Encountered, side-by-side.
    ...
    nav_left_arrow = new NeXoPageNavArrow(NeXoPageNavArrow::Side::Left);
    nav_left_arrow->on_click = [this] { GoToPage(current_page - 1); };
    nav_right_arrow = new NeXoPageNavArrow(NeXoPageNavArrow::Side::Right);
    ...
    network_probe = new NeXoNetworkProbe(this);
    ...
    connect(network_probe, &NeXoNetworkProbe::NatStatusChanged, this,
            [this, checking_style](NeXoNetworkProbe::NatStatus nat_status) {
                switch (nat_status) {
                case NeXoNetworkProbe::NatStatus::Checking:
                    nat_checking = true;
                    nat_label->setText(tr("Checking"));
                    nat_label->setStyleSheet(checking_style);
                    break;
                case NeXoNetworkProbe::NatStatus::Open:
                    nat_checking = false;
                    ...
                    nat_label->setStyleSheet(QStringLiteral(
                        ...
                        "border-radius: 9px; padding: 2px 10px;"));
                    break;
                case NeXoNetworkProbe::NatStatus::Strict:
                    nat_checking = false;
                    ...
                    nat_label->setStyleSheet(QStringLiteral(
                        ...
                        "border-radius: 9px; padding: 2px 10px;"));
                    break;
                case NeXoNetworkProbe::NatStatus::Unknown:
                    nat_checking = false;
    ...
    connect(network_probe, &NeXoNetworkProbe::PingResult, this, [this](int ms) {
        ping_checking = false;
    ...
    connect(add_button, &QPushButton::clicked, this, &NeXoAccountDialog::OnAdd);
    connect(friend_code_input, &QLineEdit::returnPressed, this, &NeXoAccountDialog::OnAdd);

    header_name->setText(QString::fromStdString(Common::NeXoAccount::GetUsername()));
    header_code->setText(QString::fromStdString(Common::NeXoAccount::GetFriendCode()));
    ...
    connect(controller, &NeXoController::StatusChanged, this,
    ...
    connect(&refresh_timer, &QTimer::timeout, this, &NeXoAccountDialog::RefreshFriends);
    connect(&refresh_timer, &QTimer::timeout, this, &NeXoAccountDialog::RefreshHistory);
    connect(&refresh_timer, &QTimer::timeout, this, &NeXoAccountDialog::RefreshCloudSaveTab);
    connect(&refresh_timer, &QTimer::timeout, this, &NeXoAccountDialog::RefreshPlayers);
    refresh_timer.start();
...
#ifdef ENABLE_WEB_SERVICE
    std::thread{[this, guard = QPointer<NeXoAccountDialog>(this)] {
        ...
        QMetaObject::invokeMethod(
            ...
            [this, guard, image = profile.image_base64] {
                ...
                const QPixmap avatar = NeXo::AvatarCache::Get("self", image, 256);
                if (!avatar.isNull()) {
...
NeXoAccountDialog::~NeXoAccountDialog() {
    hid_core.SetGuestInputSuspended(false);
}

bool NeXoAccountDialog::eventFilter(QObject* watched, QEvent* event) {
...
void NeXoAccountDialog::keyPressEvent(QKeyEvent* event) {
...
void NeXoAccountDialog::resizeEvent(QResizeEvent* event) {
    QDialog::resizeEvent(event);
    UpdateHeroSizing();
}

void NeXoAccountDialog::showEvent(QShowEvent* event) {
...
void NeXoAccountDialog::UpdateHeroSizing() {
...
void NeXoAccountDialog::GoToPage(int index) {
    if (!pages_stack) {
        return;
    }
...
void NeXoAccountDialog::WireControllerNav() {
    ...
    connect(controller_navigation, &ControllerNavigation::navigated, this,
            &NeXoAccountDialog::OnDirectionalNav);
    connect(controller_navigation, &ControllerNavigation::activated, this,
            &NeXoAccountDialog::OnControllerActivate, Qt::QueuedConnection);
...
void NeXoAccountDialog::UpdatePageNav() {
...
void NeXoAccountDialog::OnDirectionalNav(int dx, int dy) {
...
void NeXoAccountDialog::OnControllerActivate() {
    QWidget* focused = QApplication::focusWidget();
    if (!focused || !isAncestorOf(focused)) {
        return;
    }
...
bool NeXoAccountDialog::ConfirmAction(const QString& title, const QString& message,
...
void NeXoAccountDialog::ActivateCurrentRow(QListView* view) {
    ...
    if (view == outgoing_requests_view) {
        const std::string code =
            index.data(NeXoFriendItem::FriendCodeRole).toString().toStdString();
        Common::NeXoOutgoingRequests::Remove(code);
    ...
    if (is_request) {
        ...
        RunAsync([pid] { return WebService::NeXoApi::AcceptFriend(pid); });
    } else {
        const QString name = index.data(NeXoFriendItem::NameRole).toString();
        ...
        RunAsync([pid] { return WebService::NeXoApi::RemoveFriend(pid); });
    }
#endif
}

void NeXoAccountDialog::ActivateHistoryRow(const QModelIndex& index) {
    if (!index.isValid() || !controller) {
        return;
    }
    const QString title_id_hex = index.data(NeXoHistoryItem::TitleIdRole).toString();
...
void NeXoAccountDialog::OnHistoryViewClicked(const QModelIndex& index) {
    ActivateHistoryRow(index);
}

void NeXoAccountDialog::OnChangeAvatar() {
#ifdef ENABLE_WEB_SERVICE
    ...
    std::thread{[this, image_base64, guard = QPointer<NeXoAccountDialog>(this)] {
        const std::string error = WebService::NeXoApi::PushProfilePicture(image_base64);
        if (!guard) {
            return;
        }
        QMetaObject::invokeMethod(
...
void NeXoAccountDialog::OnChangeBackground() {
...
void NeXoAccountDialog::OnRemoveBackground() {
    std::error_code ec;
...
void NeXoAccountDialog::ApplyBackground(const QPixmap& pixmap) {
    static_cast<HeaderCard*>(header_card)->SetBackgroundImage(pixmap);
}

void NeXoAccountDialog::LoadSavedBackground() {
...
void NeXoAccountDialog::OnEditUsername() {
#ifdef ENABLE_WEB_SERVICE
    ...
    std::thread{[this, new_name, guard = QPointer<NeXoAccountDialog>(this)] {
        const std::string error = WebService::NeXoApi::SetUsername(new_name);
        if (!guard) {
            return;
        }
        QMetaObject::invokeMethod(
            ...
            [this, guard, error, new_name] {
                ...
                if (error.empty()) {
                    Common::NeXoAccount::Save(Common::NeXoAccount::GetPid(), new_name,
                                                  Common::NeXoAccount::GetFriendCode(),
                                                  Common::NeXoAccount::GetToken());
...
void NeXoAccountDialog::SetBusy(bool busy) {
    add_button->setEnabled(!busy);
}

u64 NeXoAccountDialog::SelectedPid(const QModelIndex& index) const {
    return index.isValid() ? index.data(NeXoFriendItem::PidRole).toULongLong() : 0;
}

void NeXoAccountDialog::ApplyFriendFilter(const QString& text) {
    for (int row = 0; row < friends_model->rowCount(); ++row) {
        const QString name =
            friends_model->index(row, 0).data(NeXoFriendItem::NameRole).toString();
        friends_view->setRowHidden(row, !text.isEmpty() && !name.contains(text, Qt::CaseInsensitive));
    }
}

void NeXoAccountDialog::UpdateRequestsBadge(int count) {
    if (count <= 0) {
        requests_badge->hide();
        UpdateDashboard();
        return;
...
void NeXoAccountDialog::UpdateDashboard() {
    int online = 0;
    int in_game = 0;
    for (int row = 0; row < friends_model->rowCount(); ++row) {
        const int presence =
            friends_model->index(row, 0).data(NeXoFriendItem::PresenceRole).toInt();
        if (presence == 1) {
            ++online;
        } else if (presence == 2) {
            ++online;
    ...
    if (request_count > 0) {
        ...
        const QString first_name =
            requests_model->index(0, 0).data(NeXoFriendItem::NameRole).toString();
...
void NeXoAccountDialog::OnFriendsViewClicked(const QModelIndex& index) {
    if (!index.isValid()) {
        return;
    }
    ...
    if (view == outgoing_requests_view) {
        ...
        if (hit != NeXoFriendDelegate::ActionHit::None) {
            const std::string code =
                index.data(NeXoFriendItem::FriendCodeRole).toString().toStdString();
            Common::NeXoOutgoingRequests::Remove(code);
            outgoing_requests_model->removeRow(index.row());
    ...
    if (is_request) {
        const QString name = index.data(NeXoFriendItem::NameRole).toString();
        if (hit == NeXoFriendDelegate::ActionHit::Primary) {
            if (!ConfirmAction(tr("Friend Request"), tr("Accept %1's friend request?").arg(name),
                              tr("Yes"), tr("No"), avatar_b64)) {
                return;
            }
            ...
            RunAsync([pid] { return WebService::NeXoApi::DeclineFriend(pid); });
        }
    } else {
        const QString name = index.data(NeXoFriendItem::NameRole).toString();
        if (!ConfirmAction(tr("Remove Friend"), tr("Remove %1 from your friends list?").arg(name),
...
void NeXoAccountDialog::RunAsync(std::function<std::string()> task,
    ...
    std::thread{[this, work = std::move(task), success_cb = std::move(on_success),
                guard = QPointer<NeXoAccountDialog>(this)] {
...
void NeXoAccountDialog::OnAdd() {
    ...
    RunAsync([code] { return WebService::NeXoApi::AddFriendByCode(code); }, [this, code] {
        Common::NeXoOutgoingRequests::Add(code);
        if (controller) {
...
void NeXoAccountDialog::RefreshFriends() {
#ifdef ENABLE_WEB_SERVICE
    SetBusy(true);
        ...
        QMetaObject::invokeMethod(
            ...
            [this, guard, list = std::move(fetched)]() mutable {
                ...
                const u64 friends_prev_pid =
                    friends_prev.isValid() ? friends_prev.data(NeXoFriendItem::PidRole).toULongLong() : 0;
                ...
                const u64 requests_prev_pid =
                    requests_prev.isValid() ? requests_prev.data(NeXoFriendItem::PidRole).toULongLong() : 0;
                ...
                const QString outgoing_prev_code =
                    outgoing_prev.isValid() ? outgoing_prev.data(NeXoFriendItem::NameRole).toString()
                                            : QString{};
                ...
                const auto rank = [&](const WebService::NeXoApi::Friend& f) -> int {
                ...
                std::stable_sort(list.friends.begin(), list.friends.end(),
                                 [&](const WebService::NeXoApi::Friend& a, const WebService::NeXoApi::Friend& b) {
                                     const int ra = rank(a);
                                     const int rb = rank(b);
                                     if (ra != rb) {
                                         return ra < rb;
                ...
                for (const auto& entry : list.friends) {
                    const QString game =
                        controller ? controller->ResolveGameName(entry.app_id, entry.app_name)
                                   : QString{};
                    friends_model->appendRow(new NeXoFriendItem(
                ...
                for (const auto& entry : list.requests) {
                    requests_model->appendRow(new NeXoFriendItem(
                        entry.pid, QString::fromStdString(entry.name),
                ...
                Common::NeXoOutgoingRequests::PruneAccepted(accepted_codes);

                outgoing_requests_model->clear();
                for (const auto& entry : Common::NeXoOutgoingRequests::Get()) {
                    outgoing_requests_model->appendRow(new NeXoFriendItem(
                ...
                if (friends_had_focus) {
                    for (int row = 0; row < friends_model->rowCount(); ++row) {
                        ...
                        if (idx.data(NeXoFriendItem::PidRole).toULongLong() == friends_prev_pid) {
                ...
                if (requests_had_focus) {
                    for (int row = 0; row < requests_model->rowCount(); ++row) {
                        ...
                        if (idx.data(NeXoFriendItem::PidRole).toULongLong() == requests_prev_pid) {
                ...
                if (outgoing_had_focus) {
                    for (int row = 0; row < outgoing_requests_model->rowCount(); ++row) {
                        ...
                        if (idx.data(NeXoFriendItem::NameRole).toString() == outgoing_prev_code) {
...
void NeXoAccountDialog::RefreshCloudSaveTab() {
    ...
    const bool is_running_eligible =
        running_title_id != 0 && NeXo::CompatibleTitles::Table().count(running_title_id);
    ...
    if (is_running_eligible) {
        ...
        const QPixmap icon = RoundedRectPixmap(
            NeXo::AvatarCache::Get(app_id_hex, game_icon.toStdString(), 64), 64, 10);
        if (!icon.isNull()) {
...
void NeXoAccountDialog::RebuildCloudSaveTitlePicker() {
    ...
    for (const auto& [program_id, version] : NeXo::CompatibleTitles::Table()) {
        ...
        button->setIcon(RoundedRectPixmap(
            NeXo::AvatarCache::Get(title_id_hex, icon_b64.toStdString(), 48), 48, 8));
...
void NeXoAccountDialog::ProbeCloudSaveAvailability(u64 title_id) {
#ifdef ENABLE_WEB_SERVICE
    ...
    std::thread{[this, title_id, title_id_hex, guard = QPointer<NeXoAccountDialog>(this)] {
        const auto save = WebService::NeXoApi::PullSave(title_id_hex);
...
namespace {
QString PlayerDisplayName(const WebService::NeXoApi::LobbyPlayer& player) {
...
QString LobbyStateLine(const WebService::NeXoApi::Lobby& lobby) {
    const QString status = lobby.state_code == "searching" ? NeXoAccountDialog::tr("Looking for players")
                          : lobby.state_code == "matched"  ? NeXoAccountDialog::tr("In a match")
                                                            : QString();
    if (status.isEmpty()) {
        return NeXoAccountDialog::tr("%1 / %2 players").arg(lobby.count).arg(lobby.max);
    }
    return NeXoAccountDialog::tr("%1 / %2 players \xE2\x80\x94 %3")
        .arg(lobby.count)
        .arg(lobby.max)
        .arg(status);
}
...
void NeXoAccountDialog::RefreshPlayers() {
#ifdef ENABLE_WEB_SERVICE
    std::thread{[this, guard = QPointer<NeXoAccountDialog>(this)] {
        auto lobby = WebService::NeXoApi::GetMyLobby();
        auto recent = WebService::NeXoApi::GetRecentPlayers();
        ...
        const auto fetch_avatar = [&](u64 pid) {
            ...
            avatars[pid] = WebService::NeXoApi::GetAvatarByPid(pid);
        };
            ...
            [this, guard, lobby = std::move(lobby), recent = std::move(recent),
             avatars = std::move(avatars)]() mutable {
                ...
                for (const auto& player : lobby.players) {
                    ...
                    lobby_model->appendRow(new NeXoFriendItem(
                ...
                for (const auto& player : recent) {
                    ...
                    recent_players_model->appendRow(new NeXoFriendItem(
                        player.pid, PlayerDisplayName(player),
...
void NeXoAccountDialog::OnPlayersViewClicked(const QModelIndex& index) {
#ifdef ENABLE_WEB_SERVICE
    if (!index.isValid()) {
        return;
    }
    ...
    const QRect cell_rect = view->visualRect(index);
    const QPoint pos = view->viewport()->mapFromGlobal(QCursor::pos());
    if (delegate->HitTestActions(cell_rect, pos, false) == NeXoFriendDelegate::ActionHit::None) {
        return;
    }

    const u64 pid = SelectedPid(index);
    if (pid == 0 || pid == Common::NeXoAccount::GetPid()) {
        return;
    }
    if (!known_player_pids.contains(pid)) {
        status->setText(tr("This player isn't a known NeXo account."));
        return;
    }
    const std::string friend_code = index.data(NeXoFriendItem::FriendCodeRole).toString().toStdString();
    ...
    RunAsync([friend_code] { return WebService::NeXoApi::AddFriendByCode(friend_code); },
             [this, friend_code] {
                 Common::NeXoOutgoingRequests::Add(friend_code);
                 if (controller) {
...
void NeXoAccountDialog::ShowPlayersContextMenu(QListView* view, const QPoint& pos) {
#ifdef ENABLE_WEB_SERVICE
    ...
    const u64 pid = SelectedPid(index);
    if (pid == 0 || pid == Common::NeXoAccount::GetPid() || !known_player_pids.contains(pid)) {
        return;
    }

    const QString name = index.data(NeXoFriendItem::NameRole).toString();
    const QString avatar_b64 = index.data(NeXoFriendItem::AvatarB64Role).toString();
...
void NeXoAccountDialog::OpenReportDialog(u64 pid, const QString& name,
    ...
    reason_combo->addItem(tr("Impersonating a NeXo account"),
    ...
    RunAsync(
        [pid, reason, comment] {
            const std::string error = WebService::NeXoApi::ReportPlayer(pid, reason, comment);
...
void NeXoAccountDialog::RefreshHistory() {
#ifdef ENABLE_WEB_SERVICE
    std::thread{[this, guard = QPointer<NeXoAccountDialog>(this)] {
        auto fetched = WebService::NeXoApi::GetHistory();
        if (!fetched.ok || !guard) {
            return;
        }
        ...
        QMetaObject::invokeMethod(
            ...
            [this, guard, list = std::move(fetched)] {
                ...
                const QString history_prev_title =
                    history_prev.isValid() ? history_prev.data(NeXoHistoryItem::TitleIdRole).toString()
                                           : QString{};
                ...
                for (const auto& entry : list.entries) {
                    ...
                    history_model->appendRow(new NeXoHistoryItem(
                ...
                if (history_had_focus) {
                    for (int row = 0; row < history_model->rowCount(); ++row) {
                        ...
                        if (idx.data(NeXoHistoryItem::TitleIdRole).toString() == history_prev_title) {
