...
class NeXoFriendDelegate;
class NeXoNetworkProbe;
class NeXoPageNavArrow;
...
class NeXoAccountDialog : public QDialog {
    Q_OBJECT
...
public:
    ...
    explicit NeXoAccountDialog(NeXoController* controller, Core::System& system,
                                   QWidget* parent = nullptr, int initial_page = kHomePage);
    ~NeXoAccountDialog() override;
...
private:
    ...
    NeXoFriendDelegate* outgoing_request_delegate;
    QListView* history_view;
    QStandardItemModel* history_model;
    QStackedWidget* history_stack;
    NeXoFriendDelegate* friend_delegate;
    NeXoFriendDelegate* request_delegate;
    ...
    NeXoPageNavArrow* nav_left_arrow = nullptr;
    NeXoPageNavArrow* nav_right_arrow = nullptr;
