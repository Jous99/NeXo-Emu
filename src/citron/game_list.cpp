...
GameList::GameList(std::shared_ptr<FileSys::VfsFilesystem> vfs_,
    ...
    connect(carousel_view, &GameCarouselView::ProfileClicked, this,
            &GameList::OpenNeXoAccountRequested);
...
void GameList::UpdateOnlineStatus() {
    ...
    NeXo::PopulationHistory::Refresh();
...
void GameList::OnOnlineStatusUpdated(const std::map<u64, std::pair<int, int>>& online_stats) {
    NeXo::LdnCounts::Set(online_stats);
...
void GameList::StartLaunchAnimation(const QModelIndex& item) {
    ...
    if (main_window) {
        main_window->OfferNeXoByamlDownload(program_id);
    }
    ...
    if (main_window && !main_window->HasPerformedBcatAutoDownload()) {
        ...
        for (int i = 0; i < item_model->rowCount(); ++i) {
            ...
            for (int j = 0; j < folder->rowCount(); ++j) {
                ...
                if (title_id != 0 && main_window->NeXoByamlRequired(title_id) &&
                    !main_window->NeXoByamlInstalled(title_id) &&
                    !main_window->NeXoByamlSkipped(title_id)) {
                    LOG_INFO(Frontend, "NeXo BCAT: auto-downloading schedule for {:016X}",
                             title_id);
                    main_window->SilentlyDownloadNeXoByaml(title_id);
                }
            }
        }
    }
...
void GameList::AddGamePopup(QMenu& context_menu, const QModelIndex& index, u64 program_id,
    ...
    if (main_window && main_window->NeXoByamlRequired(program_id) &&
        main_window->NeXoByamlDownloadEnabled()) {
    ...
    if (download_online_schedule) {
        connect(download_online_schedule, &QAction::triggered, [this, program_id]() {
            if (main_window) {
                main_window->NeXoByamlDownloadFromMenu(program_id);
            }
        });
    }
