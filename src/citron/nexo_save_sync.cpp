...
bool IsEligible(u64 title_id) {
    return NeXo::CompatibleTitles::Table().count(title_id) != 0 &&
          Common::NeXoAccount::IsLinked();
}
...
std::vector<u8> ZipDirectoryPowerShell(const FileSys::VirtualDir& dir, u64 title_id) {
    ...
    const std::filesystem::path tmp_zip =
        std::filesystem::temp_directory_path() / fmt::format("nexo_save_{:016x}.zip", title_id);
    std::error_code ec;
...
bool UnzipToDirectoryPowerShell(std::span<const u8> zip_data, const std::filesystem::path& dest,
    ...
    const std::filesystem::path tmp_zip = std::filesystem::temp_directory_path() /
                                           fmt::format("nexo_save_in_{:016x}.zip", title_id);
    {
...
void Pull(Core::System& system, u64 title_id, bool force) {
    ...
    if (!force && HasLocalContent(save_dir)) {
        LOG_INFO(Frontend, "NeXo save pull {:016X}: local save present -> kept (no overwrite)",
                 title_id);
        return;
    }
    if (!save_dir) {
    ...
    const auto zip = WebService::NeXoApi::PullSave(fmt::format("{:016x}", title_id));
    if (!zip || zip->empty()) {
        return;
    }
...
#endif
    if (applied) {
        LOG_INFO(Frontend, "NeXo save pull {:016X}: applied ({} B)", title_id, zip->size());
    }
#else
    (void)system;
    (void)title_id;
...
void UploadCaptured(u64 title_id, std::vector<u8> zip_bytes) {
    ...
    const std::string error =
        WebService::NeXoApi::PushSave(fmt::format("{:016x}", title_id), zip_bytes);
    if (!error.empty()) {
        LOG_WARNING(Frontend, "NeXo save push {:016X} failed: {}", title_id, error);
    } else {
        LOG_INFO(Frontend, "NeXo save push {:016X}: {} B", title_id, zip_bytes.size());
    }
#else
    (void)title_id;
    (void)zip_bytes;
